...
export default class UForm1 extends Component{
    constructor(props) {
        super(props);
        this.state = {
            userName: '',
    ...
    onSearchUserName = (value) => {
        ...
        this.setState({
            dataSource: dataSource.filter(item => item.custId.indexOf(value) !== -1),
            loading: false,
        })
    };
    //地址级联选择
    ...
    RangePicker_Select = (date, dateString) => {
        ...
        if(date.length===0){
            ...
            this.setState({
                timeRange: date,
                dataSource: dataSource.filter(item => (moment(item.txnDatetime.substring(0,10)) <= endtime  && moment(item.txnDatetime.substring(0,10)) >= startime) === true)
            });
        }
    };
    //渲染
